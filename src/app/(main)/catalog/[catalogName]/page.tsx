  ...
  return (
    <div className="flex flex-col items-center gap-y-4 my-4 w-full">
      ...
      <div className="grid grid-cols-3 justify-center gap-y-2 gap-x-2 2xl:grid-cols-4 w-full mx-auto">
        {filteredProducts && filteredProducts.map(({ title, discount, price, image, id, brand }) => {
          return <ProductCard id={id} discount={discount} text={title} image={image} price={price} key={title} brand={brand} />
        })}
      </div>
    </div>
  )
